...
Citizen.CreateThread(function()
    ...
    PerformHttpRequest('https://raw.githubusercontent.com/RaySist1/RaySist_documents/main/fxmanifest.lua',function(error, result, headers)
      if not result then print('raysist Documents: ^1Couldn\'t check version.^0') end
      ...
      if version_N > currentVersion_N then
        local title = "^4| raysist Documents                                                                                                          |"
...
RegisterCallback('raysist_documents:getPlayerCopies', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)

  MySQL.Async.fetchAll(
    'SELECT id, data, isCopy FROM raysist_documents WHERE ownerId = @identifier and isCopy = "1"', {
...
if Config.DocumentItemName then
  RegisterItem(Config.DocumentItemName, function(source)
    local src = source
    TriggerClientEvent("raysist_documents:useItem", src)
  end)
end

RegisterCallback('raysist_documents:getPlayerDocuments', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)

    MySQL.Async.fetchAll(
      'SELECT id, data, isCopy FROM raysist_documents WHERE ownerId = @identifier and isCopy = "0"', {
      ['@identifier'] = PlayerIdentifier
    }, function(result)
      local mappedResult = {}
      for k, v in pairs(result) do
...
RegisterCallback('raysist_documents:getDocumentTemplates', function(source, cb)
  local src = source  
  local PlayerJobName = GetPlayerJobName(src)

  MySQL.Async.fetchAll(
    'SELECT id, data FROM raysist_document_templates WHERE job = @job', {
    ['@job'] = PlayerJobName
  }, function(result)
    local mappedResult = {}
    for k, v in pairs(result) do
...
RegisterCallback('raysist_documents:createTemplate', function(source, cb, data)
  local src = source  
  local PlayerJobName = GetPlayerJobName(src)

  MySQL.Async.insert('INSERT INTO raysist_document_templates (data, job) VALUES (@data, @job)', {
			['@data'] = data,
			['@job'] = PlayerJobName
		}, function(result)
      cb(result)
...
RegisterCallback('raysist_documents:editTemplate', function(source, cb, data)
  local obj = json.decode(data)

  MySQL.Async.execute('UPDATE raysist_document_templates SET data = @data WHERE id = @id', {
			['@data'] = data,
			['@id'] = obj.id
		}, function(result)
      cb(result)
...
RegisterCallback('raysist_documents:deleteTemplate', function(source, cb, data)
  MySQL.Async.execute('DELETE FROM raysist_document_templates WHERE id = @id', {
			['@id'] = data
		}, function(result)
      cb(result)
  end)
...
RegisterCallback('raysist_documents:getPlayerData', function(source, cb)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)
...
RegisterCallback('raysist_documents:createDocument', function(source, cb, data)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)

  MySQL.Async.insert('INSERT INTO raysist_documents (data, ownerId, isCopy) VALUES (@data, @ownerId, @isCopy)', {
			['@data'] = data,
...
RegisterServerEvent('raysist_documents:createServerDocument')
AddEventHandler('raysist_documents:createServerDocument', function(data)
  local src = source
  local PlayerIdentifier = GetPlayerIdentifier(src)
...
RegisterCallback('raysist_documents:deleteDocument', function(source, cb, data)
  MySQL.Async.execute('DELETE FROM raysist_documents WHERE id = @id', {
			['@id'] = data
		}, function(result)
      cb(result)
  end)
...
RegisterServerEvent('raysist_documents:giveCopy')
AddEventHandler('raysist_documents:giveCopy', function(data, targetId)
  local src = source
  local tsrc = targetId
  ...
  MySQL.Async.insert('INSERT INTO raysist_documents (data, ownerId, isCopy) VALUES (@data, @ownerId, @isCopy)', {
    ...
    TriggerClientEvent("raysist_documents:copyGave", src, data.name)
    TriggerClientEvent("raysist_documents:copyReceived", tsrc, data.name)
  end)

end)

RegisterServerEvent("raysist_documents:receiveDocument")
AddEventHandler("raysist_documents:receiveDocument", function(data, targetId)
  local tsrc = targetId

  MySQL.Async.fetchAll('SELECT data FROM raysist_documents WHERE id = @docId', {
    ['@docId'] = data.docId,
  }, function(result)
    TriggerClientEvent("raysist_documents:viewDocument", tsrc, result[1])
  end)
end)
